...
// Type: SmartQuant.Controls.TradingTools.ModifyOrderForm
...
namespace SmartQuant.Controls.TradingTools
{
  internal class ModifyOrderForm : Form
  {
    ...
    {
      int digits = this.GetDigits(order.Instrument);
      ...
      {
        Decimal num = (Decimal) Math.Pow(0.1, (double) digits);
      ...
      switch (order.Type)
      {
        case OrderType.Market:
          ...
          this.Text = string.Format("{0} - {1} {2} [Modify]", (object) order.Instrument.Symbol, (object) order.Side, (object) order.Type);
          break;
        case OrderType.Stop:
        ...
        case OrderType.StopLimit:
          ...
          goto case OrderType.Market;
        default:
          throw new NotSupportedException("Not supported order type - " + (object) order.Type);
      }
    }
    ...
    {
      int num = 0;
      while (value * (Decimal) Math.Pow(10.0, (double) num) - Math.Truncate(value * (Decimal) Math.Pow(10.0, (double) num)) > new Decimal(0))
        ++num;
      return num;
    }
